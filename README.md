...
- **索引整体重写（未分片）**：索引已从 `data.json` 拆到插件目录下独立的 `index.json`，且向量按 **int8 量化**存储（约 273 篇笔记 ≈ 10 MB，较未量化的全精度小一个数量级）。但单次笔记改动仍会在防抖后**整体重写** `index.json`——再大时仍需分片 / 增量写索引（待办）。
- 首次升级会迁移：旧 `data.json`（内含全精度索引）在加载时一次性迁出为量化 `index.json` + 精简 `data.json`，这一次加载稍慢，之后变快。
