...
- **Vault-aware 对话面板**：侧栏里直接提问；每轮把相关旧笔记片段送进模型上下文；长请求可随时「停止」解锁面板。
...
- **设置页**：折叠分区；三套 OpenAI 兼容端点（文本 / 图像 / 嵌入）+ key + 模型，每套都能「检测」端点实际可用模型并下拉选择（文本可一键「测试」连通、嵌入按真实试嵌入筛选）；笔记目录 / 标签；可编辑的提示词。密钥只存本地 `data.json`（已 gitignore，不入库）；若你同步 `.obsidian`，密钥会随之同步，详见下方隐私边界。
