...
- **把你忘了的旧笔记撞到眼前**：每轮检索 vault，显式列出「相关旧笔记」，让第二大脑的「联想」真正发生，而不是悄悄塞进模型上下文。
...
- **对话草稿不静默丢失**：最多 3 个对话面板各自保留草稿；关闭面板只是暂停，再打开可选择恢复草稿或新建空对话，面板内也可显式「清空」当前草稿。
...
- **显式触发的三件事**（按需，不自动）：
  ...
  - `存为笔记`：把对话综述成结构化笔记，双链回相关旧笔记；保存时冻结当轮对话、来源、概念图和配图，避免弹窗期间的新内容串进同一篇。
...
Cobrain has been submitted for Obsidian Community Plugins review. After approval:
...
Until approval, use manual installation for testing unreleased versions.
...
Cobrain 已提交 Obsidian 社区插件审核。审核通过后：
...
- **换设备安装**：官方社区插件审核通过后，直接在每台设备安装插件即可；审核通过前，可继续用 vault 同步 `.obsidian/plugins/cobrain/` 或 BRAT 安装。移动端 Obsidian 需**重启/强退**才会加载新同步来的 `main.js`。
...
- 对话上下文目前只把最近 20 条消息发给模型；更早消息仍保留在草稿和面板里，但不会继续进入本轮模型上下文，面板会显示被截断的条数。
