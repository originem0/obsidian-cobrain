...
- **索引文件整体重写**：索引已从 `data.json` 拆到插件目录下独立的 `index.json`（设置保存与索引持久化彻底解耦，改设置不再重写索引），但单次笔记改动仍会在防抖后整体重写 `index.json`。vault 很大时该文件可达上百 MB，写入开销不小——根治需要分片 / 增量写索引（待办）。
- 首次升级到本版本时，旧的 `data.json`（内含索引）会在插件加载时一次性迁移成 `index.json` + 精简的 `data.json`，大索引下这一次加载会稍慢。
